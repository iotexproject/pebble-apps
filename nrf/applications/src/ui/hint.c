...
extern  void setHost(uint8_t **host, uint8_t *port, uint8_t *net);
...
void initBrokeHost(void) {
    ...
    if(!useLocalHost()) {
        ...
        setHost(&pmqttBrokerHost, &mqtt_port, &pebbleContractNet);
        printk("host:%s, port:%d, net:%d \n", pmqttBrokerHost, mqtt_port, pebbleContractNet);
    }
    buf[0] = 0xFF;
