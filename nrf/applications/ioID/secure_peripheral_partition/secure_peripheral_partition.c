...
struct tfm_pebble_service_export_args {
...
static int crypto_export_key(const uint8_t *data, size_t data_length, uint8_t *export)
{
	uint8_t ecdsa_k1_encrypted_key[64];
	uint8_t ecdsa_k1_decrypted_key[64];
	// uint8_t ecdsa_bin_key[32];
	...
	hexStr2Bin(data, data_length,ecdsa_k1_encrypted_key);
	cc3xx_decrypt(ecdsa_k1_decrypted_key, ecdsa_k1_encrypted_key);
	hexStr2Bin(ecdsa_k1_decrypted_key, 64, export);
...
static void spp_export(void)
{
	...
	struct tfm_pebble_service_init_args 	pebble_input_key;
	struct tfm_pebble_service_export_args 	pebble_export_key;

	status = psa_get(TFM_SPP_PRIVATE_SIGNAL, &msg);

	input_len = psa_read(msg.handle, 0, &pebble_input_key, sizeof(pebble_input_key));

	status = crypto_export_key(pebble_input_key.buf, pebble_input_key.len, pebble_export_key.export_buf);

	psa_write(msg.handle, 0, &pebble_export_key, sizeof(pebble_export_key));
	psa_reply(msg.handle, status);
}
...
psa_status_t tfm_spp_main(void)
{
	...
	while (1) {
		...
		if (signals & TFM_SPP_INIT_SIGNAL) {
			...
			spp_ecdsa_sign();
		} else if (signals & TFM_SPP_PRIVATE_SIGNAL) {
			spp_export();			
		} else {
			psa_panic();
		}
