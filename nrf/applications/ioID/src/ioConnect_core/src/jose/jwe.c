...
#ifndef CONFIG_PSA_COMPONENT_EXTERNAL
...
char *iotex_jwe_encrypt(char *plaintext, enum KWAlgorithms alg, enum EncAlgorithm enc, char *sender, JWK *sJWK, char *recipients_kid[JOSE_JWE_RECIPIENTS_MAX], bool format)
{
    ...
    if (NULL == ciphertext) {
        printf("2.Failed to iotex_jwe_encrypt_plaintext\n");
        goto exit;
    }
#else
    size_t tag_len = PSA_AEAD_TAG_LENGTH(PSA_KEY_TYPE_AES, 256, PSA_ALG_CCM);
    ciphertext_len = strlen(plaintext) + PSA_AEAD_TAG_LENGTH(PSA_KEY_TYPE_AES, 256, PSA_ALG_CCM);
    ...
    iotex_aes_setkey_enc( NULL, cek, 256 );
    psa_status = iotex_ccm_encrypt_and_tag( NULL, strlen(plaintext), nonce, nonce_length, protected, strlen(protected), 
                    plaintext, ciphertext, ciphertext + strlen(plaintext), tag_len );                        
    if (PSA_SUCCESS != psa_status) {
        free (plaintext);
        plaintext = NULL;
        goto exit;
    ...
    size_t plaintext_size = PSA_AEAD_DECRYPT_OUTPUT_MAX_SIZE( ciphertext_len );
    char *test_plaintext = malloc(plaintext_size);
    memset(test_plaintext, 0, plaintext_size);   

    iotex_aes_setkey_dec( NULL, cek, 256 );
    psa_status = iotex_ccm_auth_decrypt( NULL, strlen(plaintext), nonce, nonce_length, protected, strlen(protected),
                      ciphertext, test_plaintext, ciphertext + strlen(plaintext), tag_len );                     
#endif    
    ...
    char rkey[65] = {0}, secret[32] = {0}, local_iv[16] = {0}, wkey[48] = {0};
    ...
    for (int i = 0; i < JOSE_JWE_RECIPIENTS_MAX; i ++) {
        ...
        jose_status_t jose_status = iotex_jwk_get_pubkey_from_jwk(iotex_registry_find_jwk_by_kid(recipients_kid[i]), rkey + 1, &rkey_len);
        if (JOSE_SUCCESS != jose_status)
            continue;
        
        psa_status = psa_raw_key_agreement(PSA_ALG_ECDH, epk_id, rkey, 65, secret, 32, &secret_len);
        if (PSA_SUCCESS != psa_status)
            continue;
        ...
        printf("Loop.5 %d ret %d\n", i, psa_status);
        if (PSA_SUCCESS != psa_status) {
            if (wrap_id)
                psa_destroy_key(wrap_id);

            printf("3.Failed to psa_cipher_encrypt ret %d\n", psa_status);
            continue;
        }
#else
        psa_status = psa_generate_random( local_iv, 16 );
        if( psa_status != PSA_SUCCESS )
            goto exit;

        memcpy(wkey, local_iv, 16);

        psa_status = iotex_aes_setkey_enc( NULL, secret, 256 );
        psa_status = iotex_aes_crypt_cbc( NULL, 1, 32, local_iv, cek, wkey + 16 );
        if (PSA_SUCCESS != psa_status)
            continue;
#endif
...
char *iotex_jwe_decrypt(char *jwe_serialize, enum KWAlgorithms alg, enum EncAlgorithm enc, char *sender, JWK *sJWK, char *recipients_kid)
{
    ...
    jose_status_t status = iotex_jwk_get_pubkey_from_jwk(epk, epk_pub + 1, &epk_pub_len);
    ...
    size_t  secret_len = 0;
    uint8_t secret[32] = {0};    
    psa_status_t psa_status = psa_raw_key_agreement(PSA_ALG_ECDH, key_id, epk_pub, 65, secret, 32, &secret_len);
    if (PSA_SUCCESS != psa_status)
        goto exit_8;
    ...
    printf("iotex_jwe_decrypt.21.2[%d] ret %d\n", cek_len, psa_status);    
    if (PSA_SUCCESS != psa_status) 
        goto exit_8;
#else
    psa_status = iotex_aes_setkey_enc( NULL, secret, 256 );
    psa_status = iotex_aes_crypt_cbc( NULL, 0, 32, encrypted_key, encrypted_key + 16, cek );
    if (PSA_SUCCESS != psa_status)
        goto exit_8;
#endif
    ...
    iotex_aes_setkey_enc( NULL, cek, 256 );
    psa_status = iotex_ccm_auth_decrypt( NULL, ciphertext_length, iv, iv_length, 
                        protected_item->valuestring, strlen(protected_item->valuestring),
                        ciphertext_tag, plaintext, tag, tag_length );
    if (PSA_SUCCESS != psa_status) {
        free (plaintext);
        plaintext = NULL;
    }
