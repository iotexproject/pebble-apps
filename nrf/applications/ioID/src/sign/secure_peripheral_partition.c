...
psa_status_t export_private(uint8_t* encrypted_key, uint32_t size, uint8_t* export_key)
{
	psa_status_t status;
	psa_handle_t handle = TFM_SPP_PRIVATE_HANDLE;
	struct tfm_pebble_service_init_args  	input_args;
	struct tfm_pebble_service_export_args  	output_args;
	psa_invec in_vec;
	psa_outvec out_vec;
	...
	memcpy(export_key, output_args.export_buf, 32);
