...
static int otaFinish = 0;

int check_ota_status_complete()
{
    return otaFinish;
}
...
void startOTA(uint8_t *url) {
...
#else
    memset(otaHost, 0, sizeof(otaHost));
    ...
    LOG_INF("Parse host : %s, file : %s", otaHost, otaFile);
#endif

    if (0 == otaHost[0] || 0 == otaFile[0])
        return;
