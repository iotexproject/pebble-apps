...
int SensorPackage(uint16_t channel, uint8_t *buffer)
{
...
#ifdef IOTEX_PEBBLE_SENSOR_DATA_DISPLAY_ENABLE        
        printf("GPS : %d.%d\n", sensordat.latitude, sensordat.longitude);
#endif        
    }
...
#ifdef IOTEX_PEBBLE_SENSOR_DATA_DISPLAY_ENABLE        
            printf("Gyroscope [%d] : %d\n", i, sensordat.gyroscope[i]);
#endif            
        }
    }
    ...
    /* Add timestamp */
#if 1    
    uint_timestamp = iotex_pal_sprout_sensor_data_timestamp_get();
#else
    uint_timestamp = 0;
#endif
#ifdef IOTEX_PEBBLE_SENSOR_DATA_DISPLAY_ENABLE        
    ...
    *(uint32_t*)buffer = BinPackage_PackageType_DATA;
    ...
    for (int i = 0; i < sinLen; i++) {
        printf("%02x", esdaSign[i]);
    }
    printf("\n");    

    uint8_t signature_str[130] = {0};
    iotex_utils_convert_hex_to_str(esdaSign, sinLen, signature_str);
    printf("Signature : %s\n", signature_str);
#endif    
