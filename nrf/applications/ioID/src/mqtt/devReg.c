...
int SignAndSend(void)
{
    ...
    LOG_INF("uint_timestamp: %d, walletAddr:%s\n", uint_timestamp, walletAddr);
    // confirmAdd.owner.size = hexStr2Bin(walletAddr + 2, confirmAdd.owner.bytes);
    confirmAdd.owner.size = iotex_utils_convert_str_to_hex(walletAddr + 2, confirmAdd.owner.bytes);
    ...
    iotex_pal_crypt_ecdsa_sign(confirmAdd.owner.bytes, confirmAdd.owner.size + 4, esdaSign, &sinLen);
