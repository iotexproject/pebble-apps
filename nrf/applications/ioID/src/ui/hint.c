...
static void _devReg(void)
{
    int ret = 0;

    char *deviceDID    = iotex_pal_jose_device_did_get();
    char *deviceDIDDoc = iotex_pal_jose_generate_diddoc();
    ...
    ssd1306_clear_screen(0);
    /*  main menu */
    for (int i = 0; i < sizeof(devRegStr)/sizeof(devRegStr[0]); i++) {
        dis_OnelineText(i, ALIGN_LEFT, devRegStr[i], DIS_NORMAL);
    }    
#endif

    iotex_pal_device_register_init(deviceDID, deviceDIDDoc, iotex_pal_crypt_get_signature_key_id());
    ...
    while (true) {
        ...
        ret = iotex_pal_device_register_loop();
        if (1 == ret)
            break;;
...
static int ota_process = 0;

int get_ota_process_status(void)
{
    return ota_process;
}
...
void waitForOtaOver(void) 
{
    ...
    while (!check_ota_status_complete()) {
        k_sleep(K_MSEC(300));
...
// #define IOTEX_PEBBLE_OTA_TEST
void appEntryOTAProcess()
{
    if (0 == ota_process)
        return;
