...
#define  IS_USE_INDICATOR	(patternIndex > ALL_UI_MASK)
#define  INDICATOR_INDEX	(patternIndex-UI_WAIT_FOR_WALLET)
...
static const struct led_color  LedIndicator[] =
{
	UI_LED_COLOR_BLUE,
	UI_LED_COLOR_YELLOW,
	UI_LED_COLOR_WHITE,
...
static void work_handler(struct k_work *work)
{
	...
	if(IS_USE_INDICATOR)
	{		
		//printk("INDICATOR_INDEX:%d\n", INDICATOR_INDEX);
		for (size_t i = 0; i < ARRAY_SIZE(leds.color.c); i++) {
			leds.color.c[i] = LedIndicator[INDICATOR_INDEX].c[i];			
		}
		pwm_out(led, &leds.color);	
		k_delayed_work_submit(&leds.work, K_MSEC(1000));
	}
	else {
		for (size_t i = 0; i < ARRAY_SIZE(leds.color.c); i++) {
