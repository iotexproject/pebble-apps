...
#define   KEY_PAIR_FLAG		0xAA55CC33

static u8_t  RSAPubUpload = 3;
...
{   
	...
	memset(&key_pair, 0, sizeof(RSA_KEY_PAIR));
	if(GetRSAkeypair(&key_pair)){
		printk("Get key pair error \n");
		goto exit;
	}
    ...
    if( ( ret = mbedtls_mpi_read_string( &N , 16, key_pair.RSA_N ) )  != 0 ||
        ( ret = mbedtls_mpi_read_string( &E , 16, key_pair.RSA_E ) )  != 0 ||
        ( ret = mbedtls_mpi_read_string( &D , 16, key_pair.RSA_D ) )  != 0 ||
        ( ret = mbedtls_mpi_read_string( &P , 16, key_pair.RSA_P ) )  != 0 ||
        ( ret = mbedtls_mpi_read_string( &Q , 16, key_pair.RSA_Q ) )  != 0 ||
        ( ret = mbedtls_mpi_read_string( &DP , 16, key_pair.RSA_DP ) ) != 0 ||
        ( ret = mbedtls_mpi_read_string( &DQ , 16, key_pair.RSA_DQ ) ) != 0 ||
        ( ret = mbedtls_mpi_read_string( &QP , 16, key_pair.RSA_QP ) ) != 0 )
    {
    ...
    ret = mbedtls_rsa_pkcs1_decrypt( &rsa, mbedtls_ctr_drbg_random,
                                            &ctr_drbg_ctx, MBEDTLS_RSA_PRIVATE, &i,
                                            input, out, out_len );
    if( ret != 0 )
    {
...
int RSA_gen_key(RSA_KEY_PAIR *pout)
{
    int ret = 1;
    int exit_code = 1;
    mbedtls_rsa_context rsa;
	...
	size_t n;
	IOTEX_CRYPTO_DEBUG_OUTPUT("start\n");
    ...
    IOTEX_CRYPTO_DEBUG_OUTPUT( " ok\n  . Generating the RSA key [ %d-bit ]...", KEY_SIZE ); 

    if( ( ret = mbedtls_rsa_gen_key( &rsa, mbedtls_ctr_drbg_random, &ctr_drbg_ctx, KEY_SIZE,
                                     EXPONENT ) ) != 0 )
    {
	...
	IOTEX_CRYPTO_DEBUG_OUTPUT("N = %s\n\n",s);
	sprintf(pout->RSA_N, "%s", s);
	memset( s, 0, sizeof( s ) );
	mbedtls_mpi_write_string(&E, 16, s, sizeof( s ) - 2, &n );
	IOTEX_CRYPTO_DEBUG_OUTPUT("E = %s\n\n",s);
	sprintf(pout->RSA_E, "%s", s);

    IOTEX_CRYPTO_DEBUG_OUTPUT( " ok\n  . Exporting the private key in rsa_priv.txt...\n\n" );    

	memset( s, 0, sizeof( s ) );
	mbedtls_mpi_write_string(&D, 16, s, sizeof( s ) - 2, &n );
	IOTEX_CRYPTO_DEBUG_OUTPUT("D = %s\n\n",s);
	sprintf(pout->RSA_D, "%s", s);
	memset( s, 0, sizeof( s ) );
	mbedtls_mpi_write_string(&P, 16, s, sizeof( s ) - 2, &n );
	IOTEX_CRYPTO_DEBUG_OUTPUT("P = %s\n\n",s);
	sprintf(pout->RSA_P, "%s", s);
	memset( s, 0, sizeof( s ) );
	mbedtls_mpi_write_string(&Q, 16, s, sizeof( s ) - 2, &n );
	IOTEX_CRYPTO_DEBUG_OUTPUT("Q = %s\n\n",s);
	sprintf(pout->RSA_Q, "%s", s);
	memset( s, 0, sizeof( s ) );
	mbedtls_mpi_write_string(&DP, 16, s, sizeof( s ) - 2, &n );
	IOTEX_CRYPTO_DEBUG_OUTPUT("DP = %s\n\n",s);
	sprintf(pout->RSA_DP, "%s", s);
	memset( s, 0, sizeof( s ) );
	mbedtls_mpi_write_string(&DQ, 16, s, sizeof( s ) - 2, &n );
	IOTEX_CRYPTO_DEBUG_OUTPUT("DQ = %s\n\n",s);
	sprintf(pout->RSA_DQ, "%s", s);
	memset( s, 0, sizeof( s ) );
	mbedtls_mpi_write_string(&QP, 16, s, sizeof( s ) - 2, &n );
	IOTEX_CRYPTO_DEBUG_OUTPUT("QP = %s\n\n",s);
	sprintf(pout->RSA_QP, "%s", s);
...
{
	RSA_KEY_PAIR key_pair;	
	iotex_local_storage_load(SID_RSA_KEY_PAIR,&key_pair,sizeof(RSA_KEY_PAIR));
	if(key_pair.flag != KEY_PAIR_FLAG)
	{
		if(!RSA_gen_key(&key_pair))
		{			
			key_pair.flag = KEY_PAIR_FLAG;					
			iotex_local_storage_save(SID_RSA_KEY_PAIR,&key_pair, sizeof(RSA_KEY_PAIR));
		}
	}
	return 0 ;
}
...
{
	if((!iotex_local_storage_load(SID_RSA_KEY_PAIR, pkey,sizeof(RSA_KEY_PAIR))) && (pkey->flag == KEY_PAIR_FLAG))
	{	
		return  0;
	}
	else{
...
bool  RSAUpOver(void)
{
	return (RSAPubUpload == 0 ? true : false);
}
