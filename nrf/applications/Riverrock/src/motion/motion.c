...
static int accelerometer_poll(motion_acceleration_data_t *sensor_data) {
    ...
    err = sensor_channel_get(accel_dev, SENSOR_CHAN_ACCEL_Y, &accel_data[1]);
    if (err) {
        LOG_ERR("sensor_channel_get failed");
        return err;
    }
    err = sensor_channel_get(accel_dev, SENSOR_CHAN_ACCEL_Z, &accel_data[2]);
    if (err) {
        LOG_ERR("sensor_channel_get failed");
        return err;
    }
...
static int get_orientation(motion_orientation_state_t *orientation, motion_acceleration_data_t *acceleration_data) {
...
int motion_init_and_start(struct k_work_q *work_q, motion_handler_t motion_handler) {
