...
static void sampling_and_store_sensor_data(void) {
    ...
    if (iotex_mqtt_is_need_sampling()) {
        ...
        */
        ...
        if (!iotex_mqtt_sampling_data_and_store(iotex_mqtt_get_data_channel(), mqttPubBuf)) {
...
void main(void)
{
    ...
    while(true){
        if(!iotex_mqtt_is_bulk_upload() || !iotex_mqtt_is_need_sampling()) {
